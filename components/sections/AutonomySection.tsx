...
const icons = [
  <svg key="key" aria-hidden="true" className="h-6 w-6 text-ef-teal" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
  </svg>,
  <svg key="book" aria-hidden="true" className="h-6 w-6 text-ef-teal" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
  </svg>,
  <svg key="chat" aria-hidden="true" className="h-6 w-6 text-ef-teal" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
  </svg>,
  <svg key="shield" aria-hidden="true" className="h-6 w-6 text-ef-teal" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
          ...
          {cards.map((item, index) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-ef-teal/10 flex items-center justify-center flex-shrink-0">
                  {icons[index] || icons[0]}
                </div>

                <span className="inline-flex items-center rounded-full border border-ef-teal/30 px-3 py-1.5 text-xs font-semibold tracking-widest uppercase text-ef-teal">
                  {item.badge}
                </span>
              </div>
