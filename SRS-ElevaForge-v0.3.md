...
|---|---|---|---|---|
| ADR-010 | Paleta expandida derivada de los tonos de marca ya existentes, con valores y contraste verificados por cálculo (fórmula de luminancia relativa WCAG, no estimación visual). **4 roles de panel, 3 reutilizan tokens ya existentes, 1 token nuevo:**<br>• Panel "autoridad/cifra" → `forge-blue-deep` (`#174166`) + texto blanco → **10.58:1**.<br>• Panel "contenido claro" → `forge-bg-light` (`#E9EAF5`) + texto `forge-bg-dark` → **14.39:1** (ya en uso hoy).<br>• Panel "energía/CTA" → `forge-orange-main` + texto `forge-bg-dark` → **6.13:1** (ya verificado en la corrección de contraste de Fase 5).<br>• Panel "humano/equipo" → **nuevo token `forge-peach-tint: #FFDEC2`**, derivado matemáticamente del propio `orange-main` (mismo hue 27.7°/saturación 100%, lightness subida a 0.88 — no es un color inventado, es el naranja de marca "aclarado") + texto `forge-bg-dark` → **13.51:1**. Texto de acento `forge-orange-deep` sobre este panel solo en texto grande/negrita (**3.74:1**, pasa el umbral AA de 3:1 para texto grande, no el de 4.5:1 para texto normal — regla operativa: cuerpo de texto siempre `forge-bg-dark` en este panel). | Reproduce el ritmo de paneles alternados del modelo de referencia (§29) con el mínimo de tokens nuevos posible (1 de 4) — mínimo riesgo de deriva de marca, máxima coherencia con el naranja/azul ya establecido, y cero ambigüedad de implementación porque cada combinación fondo/texto ya viene con su ratio calculado, no queda para verificar después | Importar la paleta literal de CI&T (Carnation/Sail/Mauve/Tyrian Purple/East Bay) — descartado explícitamente: sería copiar identidad de un tercero (violaría la exclusión de ADR-007) y diluiría la marca ya construida de ElevaForge. También se descartó un tono pastel elegido "a ojo" (ej. `#FBEADD`) en favor de derivarlo matemáticamente del propio `orange-main`, para que quede documentado por qué es ese valor y no otro | **Aceptada** (el cliente delegó el ajuste final a criterio de ingeniería, 2026-07-22) — lista para implementar |
